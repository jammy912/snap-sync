...
三個分頁：**拍照 / 佇列 / 設定**。目錄選擇不是分頁，而是從拍照畫面
點「上傳至」拉出的浮層——現場動線是「走到定點 → 選位置 → 連拍數張 → 換位置」，
選目錄若要切分頁，戴手套單手操作時每次都得離開相機畫面才能確認自己在哪。
...
- **未選目錄**時相機畫面蓋一層「請先選擇上傳目錄」，可直接點開選單（不只是停用快門）。
- **拍完** toast 顯示「已存至 …/區域1 · 320 KB」，當下就知道存到哪，
  不必整批拍完才發現目錄選錯。
- **最近使用**的目錄以 chip 列在浮層頂端，一點即選（工地常在少數幾個區域間來回）。
- 選定後**浮層自動關閉**回到相機，全程不離開拍照畫面。
